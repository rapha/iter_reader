...
/// IterReader is implementation of io::Read and io::BufRead for an iterator of byte sequences.
...
impl <V : AsRef<[u8]>, I : Iterator<Item=V>> std::io::BufRead for IterReader<V, I> {
...
impl <V : AsRef<[u8]>, I : Iterator<Item=V>> std::io::Read for IterReader<V, I> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        if self.needs_refill() {
            self.refill();
        }
        let amt = std::cmp::min(self.buffered().len(), buf.len());
        unsafe {
            std::ptr::copy_nonoverlapping(self.buffered().as_ptr(), buf.as_mut_ptr(), amt)
        }
        self.consume(amt);
        Ok(amt)
    }
